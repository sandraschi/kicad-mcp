...
$apiBaseFile = Join-Path $Root "webapp\src\lib\api.ts"
if (Test-Path $apiBaseFile) {
    $apiContent = Get-Content $apiBaseFile -Raw
    if ($apiContent -match "127.0.0.1:(\d+)") {
        $apiPort = [int]$Matches[1]
        if ($apiPort -ne 11016) {
            throw "API_BASE in $apiBaseFile points to port $apiPort but backend serves on 11016."
        }
...
$strayExe = "$PSScriptRoot\target\release\kicad-mcp-backend.exe"
