...
const ENV_TAURI: &str = "KICAD_TAURI";
...
fn free_port(port: u16) -> bool {
    let img_kill = format!(
        "Stop-Process -Name 'kicad-mcp-backend' -Force -ErrorAction SilentlyContinue; \
         Stop-Process -Name 'kicad-mcp-native' -Force -ErrorAction SilentlyContinue; \
         taskkill /F /IM kicad-mcp-backend.exe /T 2>$null; \
         taskkill /F /IM kicad-mcp-native.exe /T 2>$null"
    );
        ...
        if i == 15 {
            let elevated = format!(
                "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                 '-NoProfile -Command \"Stop-Process -Name kicad-mcp-backend -Force -ErrorAction SilentlyContinue; \
                 taskkill /F /IM kicad-mcp-backend.exe /T 2>$null; \
                 Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                 ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
            );
